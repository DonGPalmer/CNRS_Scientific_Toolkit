...
_COMPONENTS: tuple[ComponentStatus, ...] = (
    ...
    ComponentStatus(
        ...
        NativeStatus.NATIVE_CORE,
        "CNRS-A",
        "High-level canonical CNRS-A value wrapper with native addition, negation, subtraction, and multiplication.",
        "Negation uses the finite CNRS-A representation of -1; multiplication uses general CNRS-A normalisation, not the scoped addition transducer.",
    ),
    ComponentStatus(
        ...
        NativeStatus.NATIVE_CORE,
        "CNRS-H",
        "Stores CNRS-H EGF coefficients as CNRS-A CVal objects and performs coefficient arithmetic through CNRS-A native operations.",
        "Evaluation at external numeric points remains a bridge operation.",
    ),
    ComponentStatus(
        ...
        NativeStatus.NATIVE_FINITE,
        "CNRS-H",
        "Finite-order Faà di Bruno/Bell-polynomial composition with CNRS-A coefficient arithmetic.",
        "Algorithmically CNRS-native; not a fixed finite-state digit transducer for non-polynomial outer functions.",
    ),
    ComponentStatus(
        ...
        NativeStatus.NATIVE_FINITE,
        "CNRS-A division",
        "Classifies Gaussian-rational division as integer, terminating/shifted, or eventually periodic.",
        "Sharp minimal carry-state cardinalities remain an open theory question.",
    ),
    ComponentStatus(
        "CNRS formal state",
        ...
        NativeStatus.NATIVE_LOCAL,
        "CNRS* state",
        "Theory-aligned tuple carrying CNRS-A value, explicit branch index, CNRS-H native jet, center, order, and domain metadata.",
    ),
    ComponentStatus(
